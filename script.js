...
async function loadPosts(){
  try{
    ...
    cloudPosts=Array.isArray(data)?data:[];
  }catch(error){
    console.error(
      "posts.json error:",
      error
...
function normalizeSlot(slot){
  if(String(slot).toLowerCase()==="public"){
    return "public";
  }

  const text=String(slot||"");
  const match=text.match(/slot\s*([1-9])/i);
...
function findPostByFile(name,slot){
  ...
  return getPostsForSlot(slot).find(
    post=>
      postFileName(post).toLowerCase()===target
  );
}
...
/*
  posts.json is included in counting.
  Same filename is counted only once.
*/
...
function getPrivateUsage(slot){
  const value=countPrivateFiles(slot);
...
function updatePublic(){
  ...
  if($("publicUsed")){
    $("publicUsed").textContent=used;
  }

  if($("publicAvailable")){
    $("publicAvailable").textContent=available;
  }
...
function updatePrivate(slot){
  ...
  if($("available"+slot)){
    $("available"+slot).textContent=available;
  }
...
async function restorePost(id){
  ...
  const slot=normalizeSlot(post.slot);

  /*
    Public:
    backupPassword from posts.json
...
async function refreshCloud(){
  ...
  if(
    publicPage&&
    !publicPage.classList.contains("hidden")
  ){
    renderPublic();
  }
  ...
  if(
    privatePage&&
    !privatePage.classList.contains("hidden")
  ){
    renderPrivate(currentSlot);
  }
...
function syncLocalFileCounts(){
  ...
  /*
    This initializes local counters,
    but the real displayed total is
    recalculated using posts.json.
  */
  ...
  localStorage.setItem(
    PUBLIC_USED,
    String(countPublicFiles())
  );

  localStorage.setItem(
    PRIVATE_USED,
    JSON.stringify(allPrivate)
  );
}
  ...
  ()=>{
    ...
    loadPosts()
      .then(()=>{
        ...
        /*
          posts.json is loaded before
          the first displayed statistics.
        */
...
(async function(){
  ...
  try{
    ...
    /*
      Final calculation uses posts.json.
    */
